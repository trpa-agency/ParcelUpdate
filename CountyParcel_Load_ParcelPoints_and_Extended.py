...
# -------------------------------------------------------------------
# Now update the Regional Parcels
# -------------------------------------------------------------------
# Feature classes
parcels_fc_path = r'SDE.Parcels\SDE.Parcel_Regional'
new_parcels_fc = "F:/GIS/PARCELUPDATE/Workspace/ParcelStaging.gdb/Parcel_County_Staging"
old_fc = 'Old_Feature_Class'

arcpy.MakeFeatureLayer_management(parcels_fc_path,old_fc)
...
#Delete all the rows in the Regional parcels layer
arcpy.DeleteRows_management(old_fc)

# Append the Parcels from Parcel_County_Staging.  It includes outside the boundary some of the values in parcel_Master
arcpy.Append_management(new_parcels_fc, old_fc, "NO_TEST")

#Save and stop the edit session
edit.stopOperation()
edit.stopEditing(True)
print("Regional Parcels updated successfully")